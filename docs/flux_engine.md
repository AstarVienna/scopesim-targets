...
* {math}`\varpi_\mathrm{src}` -- the pixel scale of the *source's own* image (set when you build the `Source`),
...
```{code-cell} ipython3
:tags: [hide-cell]
def train(varpi_inst, npix):
    pixel_scale = varpi_inst.to_value(u.arcsec/u.pixel)
    plate_scale = (varpi_inst / (10*u.um/u.pix)).to_value(u.arcsec/u.mm)
...
def src_grid(varpi_src, npix):
    return {"pixel_scale": varpi_src, "width": npix, "height": npix}
...
def box_integrated(varpi_src, npix, params={"x_width": 5, "y_width": 3}):
    return Box(
        spectrum=flat_spec, brightness=("V", TOTAL_FLUX), params=params
    ).to_source(src_grid(varpi_src, npix))

def box_surface_brightness(varpi_src, npix, params={"x_width": 5, "y_width": 3}):
...
With the source grid matched to the detector grid ({math}`\varpi_\mathrm{src}\equiv\varpi_\mathrm{inst}`, so there is no resampling to confound the picture),
all three representations of the same {math}`3.55\;\mathrm{Jy}` source should land on the detector as the same total:

```{code-cell} ipython3
varpi = 0.5*u.arcsec/u.pixel
opt = train(varpi, npix=15)

img_point = detected(opt, point_source())
img_box = detected(opt, box_integrated(varpi, npix=15))
img_sb = detected(opt, box_surface_brightness(varpi, npix=15))
...
The remaining question is what happens when {math}`\varpi_\mathrm{src}` and {math}`\varpi_\mathrm{inst}` *don't* match.
There are two distinct cases, and they exercise different code paths:

* {math}`\varpi_\mathrm{src}` finer than {math}`\varpi_\mathrm{inst}` -- the engine **downsamples** the input (summation-like);
* {math}`\varpi_\mathrm{src}` coarser than {math}`\varpi_\mathrm{inst}` -- the engine **interpolates / upsamples** the input, the riskier direction.

A sharp-edged box used to make this subtle to check: its edges rarely land exactly on a pixel boundary,
and a rendering bug meant the detected flux could pick up a small, {math}`\varpi_\mathrm{src}`-dependent bias from that misalignment.
...
```{code-cell} ipython3
varpi_inst = 0.2*u.arcsec/u.pixel
opt = train(varpi_inst, 55)

varpi_src_values = [0.05, 0.1, 0.2, 0.4, 0.8, 1.6]*u.arcsec/u.pixel
detected_flux = [
    float(np.nansum(detected(opt, box_integrated(varpi_src, 55))))
    for varpi_src in varpi_src_values
]

df_scales = pd.DataFrame({
    "varpi_src [arcsec/px]": varpi_src_values.value,
    "detected flux": detected_flux,
})

fig, ax = plt.subplots(figsize=(5, 3))
sns.lineplot(data=df_scales, x="varpi_src [arcsec/px]", y="detected flux",
             marker="o", ax=ax)
ax.set_xscale("log")
ax.set_xlabel(r"$\varpi_{src}$ [arcsec/px]")
...
The detected flux stays flat across more than an order of magnitude in {math}`\varpi_\mathrm{src}`.
...
Fixing the source (a point, so there is no {math}`\varpi_\mathrm{src}` at all) and sweeping only {math}`\varpi_\mathrm{inst}` should leave the detected flux untouched:
...
```{code-cell} ipython3
varpi_inst_values = [100, 200, 400, 800]*u.mas/u.pixel
fluxes = []
for varpi_inst in varpi_inst_values:
    opt = train(varpi_inst.to(u.arcsec/u.pixel), npix=25)
    fluxes.append(float(np.nansum(detected(opt, point_source()))))

df_varpi_inst = pd.DataFrame({
    "varpi_inst [mas/px]": varpi_inst_values.value,
    "detected flux": fluxes,
})

fig, ax = plt.subplots(figsize=(5, 3))
sns.lineplot(data=df_varpi_inst, x="varpi_inst [mas/px]", y="detected flux",
             marker="o", ax=ax)
ax.set_xlabel(r"$\varpi_{inst}$ [mas/px]")
ax.set_ylabel("detected flux")
...
```{code-cell} ipython3
varpi_inst = 2.0*u.arcsec/u.pixel
opt = train(varpi_inst, npix=15)
