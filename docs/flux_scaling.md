...
It assumes you have read about the target brightness syntax in {doc}`defining_brightness`.
...
The simplest target is a single {class}`~scopesim_targets.point_source.Star`.
...
fig, ax1 = plt.subplots(figsize=(7, 4), layout="compressed")
...
The dashed orange line shows the transmission curve of the filter {math}`T(\lambda)`, with its y-axis on the right side, between 0 and 1.
Initially, the spectrum is normalized internally via the [`effstim`](https://synphot.readthedocs.io/en/latest/synphot/formulae.html#effective-stimulus) in PHOTLAM units, given a flux in a filter ("bandpass").
Once it reaches the ScopeSim engine (specifically the `FieldOfView` class),
it is integrated over the selected filter (not necessarily the same one it was normalized to),
...
In terms of units, integrating over the wavelength eliminated the {math}`\lambda^{-1}` dimension, so we are left with photons per second per cm².
...
because the energy at any wavelength has been converted to a photon count.
...
For a point source, the `weight` is a simple scalar, so eq. {eq}`fdef` becomes:
...
meaning physical effects such as rotation leading to line-of-sight velocity shifts, or
different chemical signatures or temperature in different regions of the same object, cannot be represented with such a source description.
...
Instead of scaling a shared spectrum to the flux specified for a point source, it now decides _how much_ of the total flux in the scaled spectrum ends up _where_.
...
We will see in the following section why that is **not** always the case, but if we assume this for the time being, eq. {eq}`fdef` becomes:

```{math}
:label: wmaptotal

F \;=\; \sum_{ij} w_{ij} \cdot F_\mathrm{band}(\text{scaled spectrum}), \qquad \sum_{ij} w_{ij} \approx 1
```
...
It is worth mentioning at this point that multiple "2+1D" sources can be combined into an "overlay" to create a simulation scene, each with a separate spectrum.
This can be used to map different components of the same scene, where each spectrum's flux is distributed at different locations in the 2D image.
...
We can now introspect the data stored in the {class}`~scopesim.Source` object, both the 2D image and the 1D spectrum (or rather the flux stored there).
...
The spatial structure of the weightmap is given by its brightness profile {math}`p(x, y)`, implemented in the abstract {class}`~scopesim_targets.extended_source.BrightnessProfile` base class.
The exact formula behind this depends on the concrete profile.
In the example above, we have seen the simplest case: a rectangular `Box`.
While obviously not very realistic for an astrophysical object (although "boxy" galaxies exist), it is useful as a simple test case to visualize the general concept.
It is also easy to see why the naïve approach for the total of the weightmap does not work in practice:
...
For the simple `Box` case, the trivial solution would be to always force the image to be large enough,
...
Enter a simple `Gaussian` brightness profile: Mathematically only barely more complex than the `Box`, it extends to infinity.
...
And much worse, many even more realistic profiles such as the well-known {class}`~scopesim_targets.extended_source.Sersic` have no such solutions.

Thus it is much more useful to normalize the weightmap to the **intrinsic total** {math}`P\:=\:\iint p(x, y)\,\mathrm{d}x\,\mathrm{d}y` of the brightness profile instead.
...
the constant {math}`A` divides out, along with its unit, or in other words we can set {math}`A=1`.
...
To scale them correctly to the intrinsic total {math}`P` that the spectrum's flux corresponds to,
...
Since the `Box` fits comfortably within the field-of-view, its weightmap sums to exactly one.
...
A notable case relevant for e.g. calibration sources in ScopeSim is the {class}`~scopesim_targets.extended_source.Flat`, which has a constant uniform brightness.
...
which we can simplify, because we are just integrating a constant
...
If you have been following along, you will notice that this is basically identical to a `Box` whose dimensions correspond exactly to the field-of-view!
...
which is problematic if they are not exactly known at the time of defining the target.
...
This is also how well-resolved extended astronomical sources are commonly described in general, and we will get to how this works even for the integrable cases from before.
...
```{note}
For better or worse, astronomers like to give surface brightnesses in units of magnitudes per on-sky "area", e.g. {math}`\frac{\mathrm{mag}}{\mathrm{arcsec}^2}`.
...
Again we have hidden the code for the calculations by default, expand to see the details.
...
for flat_grid in flat_grids:
    wmap_sums = {
        (flat_grid["pixel_scale"], name): tg.to_source(flat_grid).fields[0].data.sum()
        for name, tg in targets.items()
    }
    spec_fluxes = {
        (flat_grid["pixel_scale"], name): band_flux(tg.to_source(flat_grid).fields[0].spectrum, band)
        for name, tg in targets.items()
    }
    totals = {
...
df.sort_index(ascending=False).style.format({
    ...
    "Spectrum flux": "{:.0f}",
    "Total (w × f)": "{:.0f}",
}).set_table_attributes("class='dataframe'").set_table_styles(tbl_sty)
```
...
```{code-cell} ipython3
fine_grid = {"pixel_scale":  0.1*u.arcsec/u.pixel, "width": 45, "height": 45}
...
This corresponds to {math}`\sim 1.5\;\mathrm{mag}` difference, which we will use for the point source.
...
### Surface brightness vs. total flux
To check that equivalent statements of total flux and surface brightness indeed produce identical results,
the example below uses the same `Box` target, once with the brightness stated as total flux
and once with that same flux converted to a surface brightness using the box's exact dimensions.
After concretization onto a grid, the resulting {class}`~scopesim.Source` objects are identical,
both in their weightmaps and in the flux their spectra carry.
...
box_sb = Box(
    spectrum=flat_spec,
    brightness=("V", sb_equivalent),
    params={"x_width": 5, "y_width": 3},
)

src_total = box_total.to_source(grid)
src_sb = box_sb.to_source(grid)

wmap_total = src_total.fields[0].data
wmap_sb = src_sb.fields[0].data
```
...
vmax = max(wmap_total.max(), wmap_sb.max())

for ax, data, title in zip(
    axes, (wmap_total, wmap_sb), ("stated as total flux", "stated as surface brightness")
):
    im = ax.imshow(data, origin="lower", extent=extent, cmap="magma", vmin=0, vmax=vmax)
    add_pixel_grid(ax, grid, major_step=2)
    ax.set_aspect("equal")
    ax.set_title(title)
...
pd.DataFrame({
    "Form": ["total-flux", "surface-brightness"],
    "Total flux": [total_flux(src_total, band), total_flux(src_sb, band)],
}).style.format({"Total flux": "{:.0f}"}
).set_table_attributes("class='dataframe'"
).set_table_styles(tbl_sty).hide(axis="index")
```
...
|-----------------------------------------------------|------------------------------------------------------------|-----------------|
| {class}`~scopesim_targets.extended_source.Box`      | {math}`w_x\,w_y`                                           | _uniform_       |
| {class}`~scopesim_targets.extended_source.Disk`     | {math}`\pi\,R_0^2`                                         | _uniform_       |
| {class}`~scopesim_targets.extended_source.Ring`     | {math}`\pi\,\left(r_\mathrm{out}^2-r_\mathrm{in}^2\right)` | _uniform_       |
| {class}`~scopesim_targets.extended_source.Gaussian` | {math}`2\pi\,\sigma_x\sigma_y`                             | peak value      |
| {class}`~scopesim_targets.extended_source.Sersic`   | {math}`2\pi n\,e^{b_\mathrm{n}}\Gamma(2n)\,b_\mathrm{n}^{-2n}(1-\mathrm{ellip})\,R_{\mathrm{eff}}^2` <br> {math}`b_\mathrm{n}=\operatorname{gammaincinv}(2n,\,0.5)` | at {math}`R_\mathrm{eff}` |
| {class}`~scopesim_targets.extended_source.Flat`     | no                                                         | _uniform_       |
...
Note that this results in much greater total integrated brightness of the `Sersic` profile,
which is correct considering the different conventions for surface brightness reference points.

### Model amplitude
A quick sidenote on model parameters: The extended source target classes use {mod}`astropy.modeling` models under the hood.
These models have a scaling parameter, usually called `amplitude`,
which has to be overridden internally to correctly scale the weightmap.
Including it in the `params` dictionary thus raises an error:
...
try:
    Box(spectrum=flat_spec, brightness=("V", TOTAL_FLUX),
        params={"x_width": 5, "y_width": 3, "amplitude": 2.0})
