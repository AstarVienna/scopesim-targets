...
It assumes you have read the target brightness syntax {doc}`defining_brightness`.
...
which is probelmatic if they are not exactly known at the time of defining the target.
...
```{TODO}
Integrate this example and rewrite.
```
...
print(f"flux (total-flux form):         {total_flux(src_total, band):.4f}")
print(f"flux (surface-brightness form): {total_flux(src_sb, band):.4f}")
```
