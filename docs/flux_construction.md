...
fluxes = []
maxs = []
for ax, grid in zip(axes.flatten(), grids):
    ...
    cbar.update_ticks()
    add_pixel_grid(ax, grid, force_major_subset=False)
...
axes[1, 0].set_xticks(np.arange(*np.ceil(extent)[:2], step=2))
axes[1, 1].set_xticks(np.arange(*np.ceil(extent)[:2], step=2))
